...
let orv_automatches_csv = "chu_lemma|orv_lemma|pos|lcs_lemma|conj_type\n";
function updateOrvMasterFile () {
  ...
  for (let i = 1; i < chu_master_arr.length; i++) {
    ...
    const unchanged_lemma_lcs = lemma_lcs;
    if(lemma_lcs != "") {
      ...
      const esl_match_idx = orv_master_arr.findIndex(x => (x[4] == "" || x[20] == "1") && x[2] == pos && x[0] == orv_esl_converted);
      if(esl_match_idx != -1) {
        ...
        console.log("Added from CHU to ORV:",unchanged_lemma_lcs, "for", chu_lemma, "matching", orv_esl_converted);
        orv_automatches_csv += chu_lemma + "|" + orv_esl_converted + "|" + pos + "|" + unchanged_lemma_lcs + "|" + conj_type + "\n";
        continue;
      }
      const chsl_match_dx = orv_master_arr.findIndex(x => (x[4] == "" || x[20] == "1") && x[2] == pos && x[0] == orv_chsl_converted);
      if(chsl_match_dx != -1) {
        ...
        orv_automatches_csv += chu_lemma + "|" + orv_chsl_converted + "|" + pos + "|" + unchanged_lemma_lcs + "|" + conj_type + "\n";
        continue;
      }
      const plain_match_idx = orv_master_arr.findIndex(x => (x[4] == "" || x[20] == "1") && x[2] == pos && x[0] == chu_lemma);
      if(plain_match_idx != -1) {
        ...
        orv_automatches_csv += chu_lemma + "|" + chu_lemma + "|" + pos + "|" + unchanged_lemma_lcs + "|" + conj_type + "\n";
        continue;
      }
    }
...
let chu_automatches_csv = "orv_lemma|chu_lemma|pos|lcs_lemma|conj_type\n";
async function updateChuMasterFile() {
  ...
  for (let i = 1; i < orv_master_arr.length; i++) {
    ...
    if(lemma_lcs != "") {
      ...
      const chu_match_idx = chu_master_arr.findIndex(x => (x[4] == "" || x[20] == "1") && x[2] == pos && x[0] == chu_converted);
      if(chu_match_idx != -1) {
        ...
        chu_automatches_csv += orv_lemma + "|" + chu_converted + "|" + pos + "|" + unchanged_lemma_lcs + "|" + conj_type + "\n";
        continue;
      }
...
async function matchLemmas() {
  ...
  fs.writeFileSync("orv_automatches_log.csv", orv_automatches_csv);
  fs.writeFileSync("chu_automatches_log.csv", chu_automatches_csv);
