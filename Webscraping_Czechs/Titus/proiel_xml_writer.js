...
const script = process.argv[2] == "glag" ? "glag" : "cyr";
const input_filename = script == "glag" ? "assem_glag_full_words_autolemmatagged.csv" : "assem_cyr_full_words_autolemmatagged.csv";
const read_stream1 = fs.createReadStream(input_filename);
...
async function readAssemFile() {
    ...
    let subtitle_no_prev = 0;
    let sentence_id_prev = 0;
    let id = 10000000;
    ...
    for await(const line of assem_file) {
        ...
        const form = row[0];
        const pos = row[1];
        const morphology = row[2];
        const lemma = row[4];
        const sentence_id = Number(row[5]);
        ...
        if(subtitle_no > subtitle_no_prev) {
            if(subtitle_no_prev > 0) {
                xml_string += "    </div>\n"
            }
...
await readAssemFile();
const output_filename = "assem_" + script + ".xml"
fs.writeFileSync(output_filename, xml_string);