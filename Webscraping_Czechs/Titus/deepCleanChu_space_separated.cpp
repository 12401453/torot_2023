...
std::string deep_clean_Cyr(std::string form_cell)
{
    ...
    for (int letno = -54; letno < 133; letno++)
    {
...
int main(int argc, char** argv)
{
    ...
    while(std::getline(inFile, line)) {
        ...
        std::string space_separated_word;
        while(std::getline(line_ss, space_separated_word, ' ')) {
            outFile << deep_clean_Cyr(space_separated_word) << ' ';
        }
        outFile << "\n";
    }
    outFile.close();
