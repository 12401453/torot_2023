...
const read_stream1 = createReadStream("assem_cyr_with_headers.txt");
...
async function writeWordsString() {
    ...
    for await(const line of assem_file) {
        ...
        if(line.startsWith("%%%%%%")) {
            citation_part = "";
            book = book_name_map[line.trim().slice(-3)];
            citation_part = "";
            continue;
        }
        else if(line.startsWith("%%%%")) {
            subtitle++;
            chapter = line.slice(13).trim();
            subtitles_file_string += title + "," + toTitleCase(book) + " " + chapter + "\n";
            continue;
        }
        else if(line.startsWith("%%")) {
            verse = line.slice(9).trim();
            citation_part = book + " " + chapter + "." + verse;
            sentence_no++;
            continue;
        }
        ...
        for(let i = first_word_index + 1 /*have to move it ahead of the first word because we always write the word only after having built the entire sequence of bullshit that comes after it and arriving at the subsequent word*/; i < split_line.length; i++) {
            ...
            if(deepClean(chunk) != "") {
                tnt_file_string += split_line[i - non_word_count] + "|" + presentation_before + "|" + presentation_after + "|" + title + "|" + String(subtitle) + "|" + citation_part + "|" + String(sentence_no) + "\n";
        ...
        if(split_line[split_line.length - 1].trim() == "") {;} 
            ...
            tnt_file_string += split_line[split_line.length - non_word_count] + "|" + presentation_before + "|" + presentation_after + "|" + title + "|" + String(subtitle) + "|" + citation_part + "|" + String(sentence_no) + "\n";
            ...
            tnt_file_string += split_line[split_line.length - 1] + "|" + presentation_before + "|" + presentation_after + "|" + title + "|" + String(subtitle) + "|" + citation_part + "|" + String(sentence_no) + "\n";
        }
...
writeFileSync("assem_words_presentation_after_book_chap_verse.csv", file_string);
writeFileSync("assem_subtitles.csv", subtitles_file_string);
