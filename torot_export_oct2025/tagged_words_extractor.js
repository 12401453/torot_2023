...
const words_filename = "PyTorch_Training_Data/"+lang_id+"_words_tagged.csv";
    ...
    if(can_parse && node.name == "token") {
        ...
        if(text_word != undefined) {
           ...
           /* else {
            ...
            bpe_training_string += deepClean(text_word) + "\n";
        }
    }
});
...
saxParser.on('end', () => {
    ...
    if(current_file_number < number_of_files) {
        ...
        fs.writeFileSync("PyTorch_Training_Data/"+lang_id+"_corpus_words_bpe_training.csv", bpe_training_string);
