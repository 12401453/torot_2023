...
let auto_lemmatised_csv_string = "torot_word|pos|morph_tag|deep_cleaned|lemma_id|lemma|sentence_id|pres_before|pres_after|text_id|subtitle_id\n";
...
async function readAutotaggedFile() {
  ...
  for await(const line of autotagged_pos_file) {
    ...
    auto_lemmatised_csv_string += word_form_raw_ats + "|" + pos + "|" + morph_tag + "|" + word_form_normalised + "|" + auto_lemma_id + "|" + lemmas_map.get(Number(auto_lemma_id)) + "|" + sentence_id + "|" + pres_before + "|" + pres_after + "|" + text_id + "|" + subtitle_id + "\n";
  }
...
async function addAutotaggedAssemToCorpus() {
  console.log("adding autolemmatised Assemanianus words to the main corpus file...");
  const assem_autotagged_words_file = readline.createInterface({input: fs.createReadStream("assemananius_autotagged_autolemmatised.csv")});
  const csv_reader = new CsvReader("|");
  let first_line = true;
  let assem_additional_words_csv = "";
  for await(const line of assem_autotagged_words_file) {
    if(first_line) {
      csv_reader.setHeaders(line);
      first_line = false;
      continue;
    ...
    csv_reader.setLine(line);
    //"torot_word|pos|morph_tag|deep_cleaned|lemma_id|lemma|sentence_id|pres_before|pres_after\n"

    const word_raw_superscripts = csv_reader.getField("torot_word");
    const pos = csv_reader.getField("pos");
    const morph_tag = csv_reader.getField("morph_tag");
    const deep_cleaned = csv_reader.getField("deep_cleaned");
    const lemma_id = csv_reader.getField("lemma_id");
    const lemma = csv_reader.getField("lemma");
    const sentence_id = csv_reader.getField("sentence_id");
    const pres_before = csv_reader.getField("pres_before");
    ...
    assem_additional_words_csv += word_raw_superscripts + "|" + pos + "|" + lemma + "|" + morph_tag + "|" + deep_cleaned + "|" + lemma_id + "|" + sentence_id + "|" + pres_before + "|" + pres_after + "|" + text_id + "|" + subtitle_id + "|" + 1 + "\n"; 

  }

  const chu_full_words_file = readline.createInterface({input: fs.createReadStream("../chu_words_full_with_titles_untagged.csv")});
  let chu_full_words_with_assem_csv = "";
  for await(const line of chu_full_words_file) {
    chu_full_words_with_assem_csv += line + "\n";
  }
  chu_full_words_with_assem_csv += assem_additional_words_csv;

  fs.writeFileSync("chu_words_full_with_assem_autotagged.csv", chu_full_words_with_assem_csv);
...
async function guessLemmas() {
  ...
  fs.writeFileSync("assemananius_autotagged_autolemmatised.csv", auto_lemmatised_csv_string);
  ...
  await addAutotaggedAssemToCorpus();
}
